...
4. **Game-parity controls.** Match the game's build controls where known. Current map (from Sean's in-game reference): Left Click applies the build mode; Right Click cycles Build → Replace → Customize → Demolish (Repair/Move intentionally omitted); R rotates (flips wall/door facing when snapped); Q/E cycle category tabs; Mouse Wheel zooms the camera by default (Shift+Wheel cycles pieces — the Admin "Wheel zooms camera" option, on by default, can be unchecked to restore the game-parity mapping where the bare wheel cycles pieces); Middle Click copies a piece; B toggles the build menu; Z/C reserved for building-set cycling. Sean can provide in-game screenshots when details are ambiguous.
...
- `incline-edge` — incline↔incline (stairs/ramps): inclined side edges → side-by-side wide staircases; horizontal high↔low edges → tiled slopes (sloped roofs/ceilings, climbing runs). Edge-length difference keeps the two from cross-matching.
...
- **Hardware bug, parked:** Shift+Wheel zoom works on MacBook trackpads but not Sean's Logitech M720 Triathlon (likely Logi Options+ remaps Shift+wheel to horizontal scroll / deltaX). When debugging: log raw wheel events from the M720; consider accepting deltaX as zoom or a keyboard fallback. (Largely mitigated since v2-dev-0031: the bare wheel zooms by default, so no Shift is needed for the common case.)
...
- **Buildable parts:** 44 of the 92 audited GLBs are wired into `v2/registry/parts.ts` — foundations + floors (square/wedge/round-corner), the wall family (straight styles 1–5, half wall, open-gap windows, window round corner, all 12 wedge walls, tall corner, inclined tall, round corners, standard/tall/garage door assemblies, and passageway), and **inclines** (straight stairs/ramps + half versions). The specialized `FloorRoundCorner_In` filler is audited but parked until curved filler placement has its own logic. Roofs, stair/ramp **corners**, pillars, railings, glass-only window inserts, etc. are audited but not yet registered.
...
- **Public About/Feedback:** the toolbar has an About / Feedback modal with trimmed Features, Roadmap, and Feedback tabs. Feedback is packaged for copy/download only; there is no backend submission endpoint yet.
